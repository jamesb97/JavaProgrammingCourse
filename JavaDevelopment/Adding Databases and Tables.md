# Adding Databases and Tables Lab
...
This lab demonstrates how to begin creating a MySQL database and elaborate on a table using Spring Boot. Apart from creating a table, will insert a sample data into the `doctor`, `doctor_available_times`, `patient`, `appointment`, and `admin` tables for the Smart Clinic Management System. Will also populate MongoDB with sample prescription documents.

## Key Terms

- **Database tables**: Structures used to store data in a database (e.g., `appointment`, `doctor`, `patient`).
- **SQL joins**: Combining rows from two or more tables based on a related column (e.g., `doctor_id`, `patient_id`).
- **Primary key**: A unique identifier for each record in a table.
- **Foreign key**: A key that links one table to another (e.g., `doctor_id` in the `appointment` table).
- **Aggregation functions**: Functions like `COUNT()`, `SUM()`, `MAX()` used to perform calculations on data (e.g., counting the number of patients a doctor sees).

## Getting Started

1. Create a database cms by executing the following command in the MySQL CLI:

`create database cms;`

2. Change the username and password in application.properties file located in `app/src/main/resources/application.properties`

3. Open the terminal using the menu `Terminal -> New Terminal` and run the following command:

`cd java-database-capstone/app`

`mvn spring-boot:run`

The above command will run the backend and create the tables automatically and you will have the following tables in the cms database:
...
To view the tables, run the following commands:
`use cms`
`show tables;`

Each table serves a specific role:

- **admin**: Stores admin user details.
- **appointment**: Stores details of patient appointments.
- **doctor**: Stores information about doctors.
- **doctor_available_times**: Tracks doctor availability slots.
- **patient**: Contains patient information.

## Insert Data into Tables

To start inserting data into the `cms` database, here's an example of the SQL insert statements:
...
2. Insert into doctor available times table
...
6. Insert into prescriptions collection in MongoDB
   `use prescriptions;`
...
In this lab, we performed various steps to create MySQL database and generated a table with additional inputs. Additionally, we added data into prescriptions collection in MongoDB.

### Next Steps

1. Test the CMS Database:

- Ensure the CMS database is correctly set up with all tables and data.
- Use the following SQL queries in the MySQL CLI to verify data was inserted correctly:

Note: For the assignment, need to submit the complete output from each command. Save the terminal output for all commands above.
...
If any data is missing or incorrect, revisit the respective `INSERT` section in the lab.
