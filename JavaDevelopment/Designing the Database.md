# Designing the Database Lab

## Overview

In this lab, will design the the data structure for the **Smart Clinic Management System** using both **MySQL** (relational) and **MongoDB** (document-based) databases.

Imagine, that you are a back-end engineer starting with a clean slate. For designing the data structure for the Smart Clinic Management System, ask these questions:
...
In this lab, create a new markdown file to **document your schema ideas and design for both SQL and MongoDB databases**. Will need to refer to these designs for building the models and database logic.
...
Before jumping into the smart clinic management system, think about what the application does.

Here is a list of real-world actions that a clinic management system should support:
...
These needs give rise to entities, such as `Patient`, `Doctor`, or `Appointment` and **relationships**(e.g., a Patient has many Appointments).

Next, go to the GitHub account and open the repository created from the template in the Create User Stories lab:

1. `java-database-capstone`

In the base directory of the repository, create a new file named:
...
Create a section in the file titled:

1. `## MySQL Database Design`

#### **Instructions:**

1. List at least 4 tables that the clinic system needs.
...
Consider defining the structure of the other tables in the format as per the given example.
...
Write your design clearly and justify choices as comments if the file is needed.
...
After completing your Markdown file with both SQL and MongoDB designs, make sure you commit it back to your GitHub repository.
